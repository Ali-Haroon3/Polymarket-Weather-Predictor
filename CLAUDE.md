...
- **Two venues, one pipeline:** `capture_prices` pulls both Polymarket and Kalshi. Kalshi (`src/api/kalshi_history.rs`, `KalshiHistoryDownloader`) maps markets into the same `WeatherMarketRow` and defaults to the DEMO env (`config::kalshi_base_url`). Its trade API has no anonymous access, so requests are RSA-PSS(SHA-256)-signed over `timestamp+method+path`; set `KALSHI_API_KEY_ID` + `KALSHI_PRIVATE_KEY_PATH` (or `KALSHI_PRIVATE_KEY_PEM`) to enable — unset ⇒ `is_available()` is false and Kalshi is silently skipped (same degrade-by-design contract as the API-keyed fetchers). Every `WeatherMarketRow`/capture carries a `source` (`"polymarket"`/`"kalshi"`); old captures without it default to `"polymarket"`.
- **Station-aware pricing (how active temp markets are actually priced):** Polymarket temperature markets resolve on the **Wunderground daily-history page of a specific airport station** — the max of the whole-degree METAR obs, NOT the NWS CLI high (often 1°F higher) and NOT gridded reanalysis. `src/stations.rs` maps each verified city to its station (traps: Denver→KBKF Buckley, not KDEN; London→EGLC City, not Heathrow; Dallas→KDAL Love, not KDFW) plus per-(city, lead k∈{0,1,2}) forecast-error `sigma`/`bias` (°C) fitted on Jan–Apr 2026 against that truth variable — the mapping reproduced 43/43 settled capture outcomes. `capture_prices::StationPricer` prices per phase: target local day over ⇒ WU max from IEM METAR obs (`data_pipeline/station_obs.rs`, σ=0.10); day-of ⇒ max(running ob max, remaining-hours live forecast at STATION coords); lead k ⇒ station-coords forecast + fitted bias/σ. Anything unmapped, Kalshi (resolves on NWS CLI — different truth, unverified), precipitation, or a failed fetch falls back to the legacy `evaluate_markets_with_forecast` path (downtown coords, σ=2.0). Refit the `stations.rs` constants from accrued captures (`forecast_high`/`forecast_sigma` are stored per snapshot) as seasons drift.
