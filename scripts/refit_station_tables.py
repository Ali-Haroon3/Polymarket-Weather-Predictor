...
Measures the drift of the CURRENT tables (stdlib only, no deps):
  ...
  3. Per-(venue, city) bias delta, empirical-Bayes shrunk toward the venue mean with weight
     ...
     RMS(z) after bias removal, clamped to [0.9, 2.0] (never sharpened much on a small
     sample, never more than doubled).
...
Usage: python3 scripts/refit_station_tables.py [--captures data/captures.jsonl] [--since 2026-07-06]
"""
import argparse
import collections
import datetime
...
def fit(train):
    ...
    for s, x in z2.items():
        scale[s] = min(2.0, max(0.9, math.sqrt(sum(x) / len(x)))) if len(x) >= 20 else 1.0
...
def main():
    ...
    ap.add_argument(
        "--since",
        default="2026-07-06",
        help="ignore captures before this date (last blend/table change)",
    )
    args = ap.parse_args()
    ...
    old, new = walk_forward(rows, recs)
    if old:
